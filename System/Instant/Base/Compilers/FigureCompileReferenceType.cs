...
namespace System.Instant
{
    ...
    public class FigureCompileReferenceType : FigureCompiler
    {
        #region Constructors

        public FigureCompileReferenceType(Figure instantFigure, IDeck<RubricBuilder> rubricBuilders) : base(instantFigure, rubricBuilders)
        {
        }
        ...
        public override void CreateFieldsAndProperties(TypeBuilder tb)
        {
            ...
            rubricBuilders.AsValues().ForEach((fp) =>
            {
                ...
                FieldBuilder fb = createField(tb, attributeAtMember, fp.Type, '_' + fp.Name);
                ...
                if (mutator != null)
                {
                    ...
                    if (args.Length == 2 && argTypes[0] == typeof(string) && argTypes[1] == typeof(object))
                    {
                        ...
                        for (int i = 0; i < length; i++)
                        {
                            ...
                            il.Emit(OpCodes.Ldarg_0); // this
                            il.Emit(OpCodes.Ldarg_2); // value
                            il.Emit(rubricBuilders[i].Type.IsValueType ? OpCodes.Unbox_Any : OpCodes.Castclass, rubricBuilders[i].Type); // type
                            il.Emit(OpCodes.Stfld, rubricBuilders[i].Field.RubricInfo); // 
                            il.Emit(OpCodes.Ret);
                        }
                    }
