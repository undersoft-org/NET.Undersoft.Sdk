...
namespace System.Series
{
    ...
    public class AlbumBase<V> : KeyedSet<V>
    {
        ...
        public AlbumBase(IEnumerable<IUnique<V>> collection, int capacity = 17, HashBits bits = HashBits.bit64) : this(capacity, bits)
        {
            list = EmptyDeck(capacity);
            foreach(var c in collection)
                Add(c);
        }
        public AlbumBase(IEnumerable<V> collection, int capacity = 17, HashBits bits = HashBits.bit64) : this(capacity, bits)
        {
            list = EmptyDeck(capacity);
            foreach(var c in collection)
                InnerAdd(c);
        }
        public AlbumBase(IList<IUnique<V>> collection, int capacity = 17, HashBits bits = HashBits.bit64) : this(capacity > collection.Count ? capacity : collection.Count, bits)
        {
            list = EmptyDeck(capacity > collection.Count ? capacity : collection.Count);
            foreach(var c in collection)
                Add(c);
        }
        public AlbumBase(IList<V> collection, int capacity = 17, HashBits bits = HashBits.bit64) : this(capacity > collection.Count ? capacity : collection.Count, bits)
        {
            list = EmptyDeck(capacity > collection.Count ? capacity : collection.Count);
            foreach(var c in collection)
                InnerAdd(c);
        ...
        protected override bool InnerAdd(ulong key, V value)
        {
            ...
            for(; ; )
            {
                ...
                if(card.Extended == null)
                {
                    /// assign new card as extended reference and increase conflicts
